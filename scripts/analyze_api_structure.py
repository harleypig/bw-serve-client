...
def analyze_api_structure(swagger_file: str) -> Dict[str, Any]:
    ...
    # Extract server info
    if 'servers' in data:
        analysis['base_url'] = data['servers']
    elif 'host' in data:
        analysis['base_url'] = {
            'host': data.get('host', ''),
    ...
    for status_code in response_patterns:
        analysis['response_patterns'][status_code] = sorted(list(response_patterns[status_code]))
...
def print_analysis(analysis: Dict[str, Any]) -> None:
    ...
    if analysis['base_url']:
        for key, value in analysis['base_url'].items():
            print(f"  {key}: {value}")
    else:
