...
class TypeGenerator:
    ...
    def _generate_enum_class(self, field_name: str, enum_values: List[Any], parent_class: str) -> str:
    ...
    def _generate_pydantic_model(self, schema_name: str, schema: Dict[str, Any], module_name: str = "") -> str:
    ...
    def _generate_imports(self, types_used: Set[str], module_name: str) -> List[str]:
        ...
        if module_name != "global_types":
            ...
            imports.append("    from .global_types import Collection, Field, Folder, Group, Status, Uris")
            imports.append("")
        
        if module_name != "item_types":
            imports.append("if TYPE_CHECKING:")
            imports.append("    from .item_types import ItemCard, ItemIdentity, ItemLogin, ItemSecureNote, ItemTemplate")
            imports.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Python types from Bitwarden Vault Management API OpenAPI specification"
    )
