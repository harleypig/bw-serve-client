        ...
        for key in obj2:
            if key not in obj1:
                ...
                if isinstance(obj1[key], dict) and isinstance(obj2[key], dict):
                    ...
                    differences.extend(find_differences(obj1[key], obj2[key], f'{path}.{key}' if path else key))
                else:
...
def main():
    ...
    parser.add_argument('--output-file', default='scripts/spec-fixes.json',
                       help='Output file path (default: scripts/spec-fixes.json)')
    parser.add_argument('--original-file', default='scripts/vault-management-api-original.json',
                       help='Original API spec file')
    parser.add_argument('--fixed-file', default='scripts/vault-management-api-fixed.json',
    ...
    comp_diffs = find_differences(original.get('components', {}), fixed.get('components', {}), 'components')
    ...
    for diff in all_differences:
        ...
        if (diff['operation'] in ['add', 'set_value'] and 
